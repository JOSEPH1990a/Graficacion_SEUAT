...
{
 // echo "<script>console.log('Debug Objects:Enviando' );</script>";
  ...
  $extension = pathinfo($info->getFilename(), PATHINFO_EXTENSION);
   if($extension == 'csv')
   {
    ...
    while(($data = fgetcsv($handle,10000,",")) !=FALSE){
        //echo "<script>console.log('Debug Objects: " .$fila. "' );</script>";
        if($data[1]!="Correo Electronico" || $data[4]!="Duracion"){
            //$horas = gmdate("H:i:s",$data[4]);
            //$time = round($data[4]);
            //$horas = sprintf('%02d:%02d:%02d', ($time/3600),($time/60%60), $time%60);
            $q = "INSERT INTO c_registros (correo,fecha,duracion,tipo,id_plantel) VALUES ('$data[1]','$data[2]','$data[4]','$data[5]','$data[6]')";
            $link->query($q);
        }else{
...
<head>
  ...
  <title>Graficacion SEUAT | Archivo</title>
                                    ...
                                    <form enctype="multipart/form-data" method="post" action="">
                                        <div class="card-body">
                                            <div class="form-group">
                                                <label for="exampleInputFile">Archivo CSV</label>
                                        ...
                                        <div class="card-footer">
                                            <button type="submit" class="btn btn-primary" value="Enviar" name="enviar">Subir</button>
                                           <!-- <button id="enlace" type="submit" href="../Controller/guardar_csv.php?cod=<?php //echo($data) ?> " class="btn btn-primary" value="Enviar" name="enviar">Subir</button>-->
                                        </div>
...
<script src="../Controller/funciones.js"></script>
</body>
</html>
